...
`git log (--online)` 查看提交日志；`git relog`查看所有的命令记录
...
`git branch` 查看分支；`git branch xx`创建分支
`gir checkout/switch <name>`切换分支
...
Q：fork 后修改，如何同步源的更新？
A：`git remote add upstream <url>`配置上游仓库；`git remote -v`查看配置是否成功；`git fetch upstream`抓取上游仓库（所有分支）的更新；`git checkout master`切换到本地主分支；`git merge upstream/master`分支合并；`git push (origin master)`推送到远程仓库
