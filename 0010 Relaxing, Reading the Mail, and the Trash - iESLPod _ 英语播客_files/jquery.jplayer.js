;(function (root, factory) {
  ...
  $.jPlayer.htmlEvent = [
    // These HTML events are bubbled through to the jPlayer event, without any internal action.
    'loadstart',
  ...
  $.jPlayer.pause = function () {
    $.each($.jPlayer.prototype.instances, function (i, element) {
      if (element.data('jPlayer').status.srcSet) {
        // Check that media is set otherwise would cause error event.
  ...
  ConvertTime.prototype = {
    ...
    time: function (s) {
      // function used on jPlayer.prototype._convertTime to enable per instance options.
      ...
      var myTime = new Date(s * 1000),
        ...
        strHour =
          this.options.timeFormat.padHour && hour < 10 ? '0' + hour : hour,
        strMin = this.options.timeFormat.padMin && min < 10 ? '0' + min : min,
        strSec = this.options.timeFormat.padSec && sec < 10 ? '0' + sec : sec,
        strTime = ''

      strTime += this.options.timeFormat.showHour
        ? strHour + this.options.timeFormat.sepHour
        : ''
      strTime += this.options.timeFormat.showMin
        ? strMin + this.options.timeFormat.sepMin
        : ''
      strTime += this.options.timeFormat.showSec
        ? strSec + this.options.timeFormat.sepSec
        : ''
  ...
  $.jPlayer.uaPlatform = function (userAgent) {
    ...
    var tablet =
      rtablet.exec(ua) || (!rmobile.exec(ua) && randroid.exec(ua)) || []
  ...
  $.jPlayer.nativeFeatures = {
    init: function () {
      ...
      var d = document,
        ...
        specOrder = ['w3c', 'moz', 'webkit', 'webkitVideo'],
        fs,
        i,
        il
  ...
  $.jPlayer.prototype = {
    ...
    options: {
      // Instanced in $.jPlayer() constructor
      ...
      repeat: function (event) {
        // The default jPlayer repeat event handler
        if (event.jPlayer.options.loop) {
          $(this)
            .unbind('.jPlayerRepeat')
            .bind(
              $.jPlayer.event.ended + '.jPlayer.jPlayerRepeat',
              function () {
                $(this).jPlayer('play')
              }
            )
...
*/
    ...
    solution: {
      // Static Object: Defines the solutions built in jPlayer.
      html: true,
      flash: true,
    },
    ...
    _init: function () {
      ...
      this.options.timeFormat = $.extend(
        {},
        $.jPlayer.timeFormat,
      ...
      $.each(this.options.supplied.toLowerCase().split(','), function (
        index1,
        value1
      ) {
      ...
      $.each(this.options.solution.toLowerCase().split(','), function (
        index1,
        value1
      ) {
      ...
      this.htmlElement.poster.onload = function () {
        // Note that this did not work on Firefox 3.6: poster.addEventListener("onload", function() {}, false); Did not investigate x-browser.
      ...
      if (this.require.audio) {
        // If a supplied format is audio
        this.htmlElement.audio = document.createElement('audio')
        this.htmlElement.audio.id = this.internal.audio.id
        this.html.audio.available =
          !!this.htmlElement.audio.canPlayType &&
          this._testCanPlayType(this.htmlElement.audio) // Test is for IE9 on Win Server 2008.
      }
      this.html.video.available = false
      if (this.require.video) {
        // If a supplied format is video
        this.htmlElement.video = document.createElement('video')
        this.htmlElement.video.id = this.internal.video.id
        this.html.video.available =
          !!this.htmlElement.video.canPlayType &&
          this._testCanPlayType(this.htmlElement.video) // Test is for IE9 on Win Server 2008.
      }
      ...
      this.flash.canPlay = {}
      $.each(this.formats, function (priority, format) {
        self.html.canPlay[format] =
          self.html[self.format[format].media].available &&
          '' !==
            self.htmlElement[self.format[format].media].canPlayType(
              self.format[format].codec
            )
        self.flash.canPlay[format] =
          self.format[format].flashCanPlay && self.flash.available
      })
      ...
      $.each(this.solutions, function (solutionPriority, solution) {
        if (solutionPriority === 0) {
          ...
          self[solution].desired =
            (self.require.audio && !audioCanPlay) ||
            (self.require.video && !videoCanPlay)
        }
      })
      ...
      this.flash.support = {}
      $.each(this.formats, function (priority, format) {
        self.html.support[format] =
          self.html.canPlay[format] && self.html.desired
        self.flash.support[format] =
          self.flash.canPlay[format] && self.flash.desired
      })
      ...
      if (this.flash.used) {
        ...
        if (
          $.jPlayer.browser.msie &&
          (Number($.jPlayer.browser.version) < 9 ||
            $.jPlayer.browser.documentMode < 9)
        ) {
          var objStr =
            '<object id="' +
      ...
      if (this.html.used && !this.flash.used) {
        // If only HTML, then emulate flash ready() call after 100ms.
        setTimeout(function () {
        ...
        function () {
          if (entity.gate) {
            ...
            if (self.status.srcSet) {
              ...
              if (
                self._validString(self.status.media.poster) &&
      ...
      if (typeof media.seekable === 'object' && media.seekable.length > 0) {
        sp =
          this.status.duration > 0
            ? (100 * media.seekable.end(media.seekable.length - 1)) /
              this.status.duration
            : 100
        cpr =
          this.status.duration > 0
            ? (100 * media.currentTime) /
              media.seekable.end(media.seekable.length - 1)
            : 0 // Duration conditional for iOS duration bug. ie., seekable.end is a NaN in that case.
      } else {
        sp = 100
        cpr = cpa
      }
    ...
    _trigger: function (eventType, error, warning) {
      // eventType always valid as called using $.jPlayer.event.eventType
      ...
      if (this.flash.gate) {
        switch (eventType) {
          ...
          case $.jPlayer.event.error:
            ...
            if (this.status.video) {
              // Set up for another try. Execute before error event.
    ...
    setMedia: function (media) {
      ...
      $.each(this.formats, function (formatPriority, format) {
        ...
        $.each(self.solutions, function (solutionPriority, solution) {
          if (
            self[solution].support[format] &&
            self._validString(media[format])
          ) {
    ...
    videoPlay: function () {
      // Handles clicks on the play button over the video poster
      this.play()
    },
    pause: function (time) {
      time = typeof time === 'number' ? time : NaN // Remove jQuery event from click handler
    ...
    mute: function (mute) {
      // mute is either: undefined (true), an event object (true) or a boolean (muted).
      mute = mute === undefined ? true : !!mute
      this._muted(mute)
    },
    unmute: function (unmute) {
      // unmute is either: undefined (true), an event object (true) or a boolean (!muted).
    ...
    volumeBar: function (e) {
      // Handles clicks on the volumeBar
    ...
    volumeBarValue: function (e) {
      // Handles clicks on the volumeBarValue
      this.volumeBar(e)
    },
    ...
    volumeMax: function () {
      // Handles clicks on the volume max
      this.volume(1)
      if (this.options.muted) {
        this._muted(false)
      }
    ...
    _cssSelectorAncestor: function (ancestor) {
      ...
      if (ancestor && this.ancestorJq.length !== 1) {
        // So empty strings do not generate the warning.
        this._warning({
          type: $.jPlayer.warning.CSS_SELECTOR_COUNT,
          context: ancestor,
          message:
            $.jPlayer.warningMsg.CSS_SELECTOR_COUNT +
      ...
      if (typeof cssSel === 'string') {
        if ($.jPlayer.prototype.options.cssSelector[fn]) {
          ...
          if (cssSel && this.css.jq[fn].length !== 1) {
            // So empty strings do not generate the warning. ie., they just remove the old one.
            this._warning({
              type: $.jPlayer.warning.CSS_SELECTOR_COUNT,
              context: this.css.cs[fn],
              message:
                $.jPlayer.warningMsg.CSS_SELECTOR_COUNT +
                this.css.jq[fn].length +
                ' found for ' +
                fn +
                ' method.',
              hint: $.jPlayer.warningHint.CSS_SELECTOR_COUNT,
    ...
    noSolution: function () {
      // Handles clicks on the error message
    ...
    _setOptions: function (options) {
      ...
      $.each(options, function (key, value) {
        // This supports the 2 level depth that the options of jPlayer has. Would review if we ever need more depth.
      ...
      switch (key) {
        ...
        case 'fullScreen':
          if (this.options[key] !== value) {
            ...
            if (!wkv || (wkv && !this.status.waitForPlay)) {
              if (!wkv) {
                // No sensible way to unset option on these devices.
        ...
        case 'size':
          if (
            ...
            this.options[key].cssClass !== value.cssClass
          ) {
            this._removeUiClass()
          }
        ...
        case 'noFullWindow':
          ...
          this.status.nativeVideoControls = this._uaBlocklist(
            this.options.nativeVideoControls
          ) // Need to check again as noFullWindow can depend on this flag and the restrict() can override it.
        ...
        case 'emulateHtml':
          if (this.options[key] !== value) {
            // To avoid multiple event handlers being created, if true already.
            this.options[key] = value
            if (value) {
              this._emulateHtmlBridge()
            } else {
    ...
    _fullscreenchange: function () {
      ...
      if (
        ...
        !$.jPlayer.nativeFeatures.fullscreen.api.fullscreenElement()
      ) {
        this._setOption('fullScreen', false)
      }
    },
    ...
    _html_setVideo: function (media) {
      this._html_setFormat(media)
      if (this.status.nativeVideoControls) {
        this.htmlElement.video.poster = this._validString(media.poster)
          ? media.poster
          : ''
      }
    ...
    _html_play: function (time) {
      ...
      if (!isNaN(time)) {
        ...
        try {
          ...
          if (
            !media.seekable ||
            (typeof media.seekable === 'object' && media.seekable.length > 0)
          ) {
    ...
    _html_pause: function (time) {
      ...
      if (time > 0) {
        // Avoids a setMedia() followed by stop() or pause(0) hiding the video play button.
        this._html_checkWaitForPlay()
      }
    },
    ...
    _getFlashPluginVersion: function () {
      ...
      if (window.ActiveXObject) {
        try {
          ...
          if (flash) {
            // flash will return null when ActiveX is disabled
        ...
        if (flash) {
          version = navigator.plugins['Shockwave Flash'].description.replace(
            /.*\s(\d+\.\d+).*/,
            '$1'
          )
        }
      ...
      $.each($.jPlayer.event, function (eventName, eventType) {
        ...
        if (nativeEvent) {
          self.element.bind(eventType + '.jPlayer.jPlayerHtml', function () {
            // With .jPlayer & .jPlayerHtml namespaces.
    ...
    _destroyHtmlBridge: function () {
      ...
      var emulated =
        $.jPlayer.emulateMethods +
        ' ' +
        $.jPlayer.emulateStatus +
        ' ' +
        $.jPlayer.emulateOptions
  ...
  $.jPlayer.errorMsg = {
    ...
    FLASH_DISABLED:
      "jPlayer's Flash fallback has been disabled by the browser due to the CSS rules you have used. Details: ", // Used in: _flashError()
    NO_SOLUTION:
      'No solution can be found by jPlayer in this browser. Neither HTML nor Flash can be used.', // Used in: _init()
    NO_SUPPORT:
      'It is not possible to play any media format provided in setMedia() on this browser using your current options.', // Used in: setMedia()
    URL: 'Media URL could not be loaded.', // Used in: jPlayerFlashEvent() and _addHtmlEventListeners()
    URL_NOT_SET:
      'Attempt to issue media playback commands, while no media url is set.', // Used in: load(), play(), pause(), stop() and playHead()
    VERSION:
      'jPlayer ' +
      $.jPlayer.prototype.version.script +
      ' needs Jplayer.swf version ' +
      $.jPlayer.prototype.version.needFlash +
      ' but found ', // Used in: jPlayerReady()
  }
...
btn1.onclick = function () {
  currentTime = document.getElementById('jp-current-time')
  currentTime += 5
}
